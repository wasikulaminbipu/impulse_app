...
void main() {
  ...
  group('AppErrorBoundary Widget Tests', () {
    testWidgets('renders child widget normally when no error occurs', (tester) async {
    ...
    testWidgets('GlobalErrorFallbackWidget renders error message gracefully', (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: GlobalErrorFallbackWidget(errorDetails: details),
        ),
      );

      expect(find.text('An unexpected display error occurred'), findsOneWidget);
      expect(find.text('The application recovered safely and prevented a crash.'), findsOneWidget);
    });

    testWidgets('AppErrorHandler initializes ErrorWidget.builder correctly', (tester) async {
